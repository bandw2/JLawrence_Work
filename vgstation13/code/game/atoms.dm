...
/atom/DblClick(location, control, params) //TODO: DEFERRED: REWRITE
	...
	//world << "------- DIR CHANGING WHEN CLICKING ------"
	...
	if(params)
		...
		if(parameters["shift"]){
			if(!isAI(usr) && !ispAI(usr))
				...
				if(ispAI(usr) && istype(src, /obj/machinery/door/airlock))
					var/mob/living/M = usr.loc// to card
					M = M.loc//to person holding card
					var/mob/living/carbon/human/H
					var/obj/machinery/door/airlock/A = usr
					...
					if(!istype(M, /mob/living/carbon/human))
						...
						H = M
						if(H && (A.check_access(H.get_active_hand()) || A.check_access(H.wear_id)))
							pAIShiftClick(usr)
							return
				else
					AIShiftClick(usr)
...
*/
	...
	// ------- PARALYSIS, STUN, WEAKENED, DEAD, (And not AI) ------- bandw2 was here
	if (((usr.paralysis || usr.stunned || usr.weakened) && !(istype(usr, /mob/living/silicon/ai)|| istype(usr, /mob/living/silicon/pai))) || usr.stat != 0)
		return
	...
	if ((!( src in usr.contents ) && (((!( isturf(src) ) && (!( isturf(src.loc) ) && (src.loc && !( isturf(src.loc.loc) )))) || !( isturf(usr.loc) )) && (src.loc != usr.loc && (!( istype(src, /obj/screen) ) && !( usr.contents.Find(src.loc) ))))))
		if (istype(usr, /mob/living/silicon/ai))
			...
			//bandw2 was here DONT FUCKING RETURN AND MAKE MY WORK ALL FOR NAUGHT, THIS TOOK 3 DAYS TO FIND! IT WAS SOLELY SPENT ON THIS
		else
			return

	// ------- 1 TILE AWAY -------
	var/t5
	// ------- AI CAN CLICK ANYTHING ------- bandw2 was here
	if(istype(usr, /mob/living/silicon/ai) || istype(usr, /mob/living/silicon/pai))
		t5 = 1
	...
	//world << " ------- ACTUALLY DETERMINING STUFF -------"
	if (((t5 || (W && (W.flags & USEDELAY))) && !( istype(src, /obj/screen) )))

		//world << "------- ( CAN USE ITEM OR HAS 1 SECOND USE DELAY ) AND NOT CLICKING ON SCREEN -------"

		if (usr.next_move < world.time)
			usr.prev_move = usr.next_move
			usr.next_move = world.time + 10
		else
			//world << "------- ALREADY USED ONE ITEM WITH USE DELAY IN THE PREVIOUS SECOND -------"
			return

		//world << "------- DELAY CHECK PASSED -------"

		if ((src.loc && (get_dist(src, usr) < 2 || src.loc == usr.loc)))

			//world << "------- CLICKED OBJECT EXISTS IN GAME WORLD, DISTANCE FROM PERSON TO OBJECT IS 1 SQUARE OR THEY'RE ON THE SAME SQUARE -------"
			...
			if ( (direct - 1) & direct)
				...
				//world << "------- CLICKED OBJECT IS LOCATED IN A DIAGONAL POSITION FROM THE PERSON -------"
				...
				if(Step_1 && Step_2)
					...
					//world << "------- BOTH CARDINAL DIRECTIONS OF THE DIAGONAL EXIST IN THE GAME WORLD -------"
						...
						for(var/obj/border_obstacle in Step_1)
							if(border_obstacle.flags & ON_BORDER)
								if(!border_obstacle.CheckExit(D, src))
									check_1 = 0
									//world << "------- YOU TRIED TO CLICK ON AN ITEM THROUGH A WINDOW (OR SIMILAR THING THAT LIMITS ON BORDERS) ON ONE OF THE DIRECITON TILES -------"
						for(var/obj/border_obstacle in get_turf(src))
							if((border_obstacle.flags & ON_BORDER) && (src != border_obstacle))
								if(!border_obstacle.CanPass(D, D.loc, 1, 0))
									//world << "------- YOU TRIED TO CLICK ON AN ITEM THROUGH A WINDOW (OR SIMILAR THING THAT LIMITS ON BORDERS) ON THE TILE YOU'RE ON -------"
									check_1 = 0
					...
					if(check_1 || check_2)
						ok = 1
						//world << "------- YOU CAN REACH THE ITEM THROUGH AT LEAST ONE OF THE TWO DIRECTIONS. GOOD. -------"
				...
				//world << "------- OBJECT IS ON A CARDINAL TILE (NORTH, SOUTH, EAST OR WEST OR THE TILE YOU'RE ON) -------"
				if(loc == usr.loc)
					ok = 1
					//world << "------- OBJECT IS ON THE SAME TILE AS YOU -------"
				else
					ok = 1
			...
			if (!( ok ))
				//world << "------- TESTS ABOVE DETERMINED YOU CANNOT REACH THE TILE -------"
				return 0

		if (!( usr.restrained() || (usr.lying && usr.buckled!=src) ))
			//world << "------- YOU ARE NOT REASTRAINED -------"

			if (W)
				//world << "------- YOU HAVE AN ITEM IN YOUR HAND - HANDLE ATTACKBY AND AFTERATTACK -------"
				if (t5)
					src.attackby(W, usr)
				if (W)
				...
				//world << "------- YOU DO NOT HAVE AN ITEM IN YOUR HAND -------"
				if (istype(usr, /mob/living/carbon/human))
					//world << "------- YOU ARE HUMAN -------"
					src.attack_hand(usr, usr.hand)
				else
					//world << "------- YOU ARE NOT HUMAN. WHAT ARE YOU - DETERMINED HERE AND PROPER ATTACK_MOBTYPE CALLED -------"
					...
					else if (istype(usr, /mob/living/silicon/ai) || istype(usr, /mob/living/silicon/robot) || istype(usr, /mob/living/silicon/pai))//bandw2 was here
						//world << "silicon pass"
			...
			//world << "------- YOU ARE RESTRAINED. DETERMINE WHAT YOU ARE AND ATTACK WITH THE PROPER HAND_X PROC -------"
			if (istype(usr, /mob/living/carbon/human))
				src.hand_h(usr, usr.hand)
				...
				src.hand_al(usr, usr.hand)
			else if (istype(usr, /mob/living/silicon/ai) || istype(usr, /mob/living/silicon/robot) || istype(usr, /mob/living/silicon/ai))//bandw2 was here
				src.hand_a(usr, usr.hand)

	else
		//world << "------- ITEM INACESSIBLE OR CLICKING ON SCREEN -------"
		if (istype(src, /obj/screen))
			//world << "------- IT'S THE HUD YOU'RE CLICKING ON -------"
			usr.prev_move = usr.next_move
		...
		//	world << "------- 2 DECISECOND DELAY FOR CLICKING PASSED -------"

			if (!( usr.restrained() ))

			//	world << "------- YOU ARE NOT RESTRAINED -------"
				if ((W && !( istype(src, /obj/screen) )))
				//	world << "------- IT SHOULD NEVER GET TO HERE, DUE TO THE ISTYPE(SRC, /OBJ/SCREEN) FROM PREVIOUS IF-S - I TESTED IT WITH A DEBUG OUTPUT AND I COULDN'T GET THIST TO SHOW UP. -------"
					src.attackby(W, usr)
					if (W)
						W.afterattack(src, usr,, params)
				else
			//		world << "------- YOU ARE NOT RESTRAINED, AND ARE CLICKING A HUD OBJECT -------"
					if (istype(usr, /mob/living/carbon/human))
						src.attack_hand(usr, usr.hand)
					else if (istype(usr, /mob/living/carbon/monkey))
						src.attack_paw(usr, usr.hand)
			...
			//	world << "------- YOU ARE RESTRAINED CLICKING ON A HUD OBJECT -------"
				if (istype(usr, /mob/living/carbon/human))
					src.hand_h(usr, usr.hand)
				else if (istype(usr, /mob/living/carbon/monkey))
					src.hand_p(usr, usr.hand)
		...
		//	world << "------- YOU ARE CLICKING ON AN OBJECT THAT'S INACCESSIBLE TO YOU AND IS NOT YOUR HUD -------"
			if((LASER in usr:mutations) && usr:a_intent == "hurt" && world.time >= usr.next_move)
		//		world << "------- YOU HAVE THE LASER MUTATION, YOUR INTENT SET TO HURT AND IT'S BEEN MORE THAN A DECISECOND SINCE YOU LAS TATTACKED -------"
...
/atom/proc/pAIShiftClick() // Opens and closes doors!
	if(istype(src , /obj/machinery/door/airlock))
